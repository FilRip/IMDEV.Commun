...
Public Class connectSQLite
    ...
    Private Shared _instance As connectSQLite
    Private _conn As SQLiteConnection
    Private _lastError As String = ""
    Private _proc As SQLiteCommand
    Private _lastServeur As String = ""

    Public Shared Function getInstance() As connectSQLite
        If (_instance Is Nothing) Then _instance = New connectSQLite
        Return _instance
    End Function

    Public Function connexion(ByVal nomFichier As String) As Boolean
        Dim chaineConnexion As String = "Data Source=<%nomFichier%>;Pooling=true;FailIfMissing=false"

        chaineConnexion = chaineConnexion.Replace("<%nomFichier%>", nomFichier)
        Try
            _lastServeur = chaineConnexion
            _conn = New SQLiteConnection(chaineConnexion)
            _conn.Open()
    ...
    Private Function copieDonnees(ByRef source As SQLiteDataReader) As unRetourRequete
    ...
    Public Function retourneDonnees(ByVal requete As String) As unRetourRequete
        Dim sa As New SQLiteCommand
        Dim src As SQLiteDataReader = Nothing
